...
# See: https://www.shivering-isles.com/helpful-shell-snippets-for-docker-testing-and-bootstrapping/
docker_image_latest_tag() { REPO=$1; [ $(echo $REPO | grep '/' | wc -l) -eq 0 ] && REPO=library/$1; wget -q -O- https://registry.hub.docker.com/v2/repositories/${REPO}/tags\?page\=1\&page_size\=250 | jq .results[].name | sort -V | sed -e 's/"//g' | grep -P "^10" | grep -P '^[[:digit:]]+((.[[:digit:]]+)?.[[:digit:]]+$)' | tail -1; }
